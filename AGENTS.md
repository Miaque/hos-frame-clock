...
- Token 只来自进程环境变量或当前工作目录的 `.env`，不通过参数传入；读取配置不修改进程环境。每个 Token 同时最多服务 `PADDLEOCR_CONCURRENCY_PER_TOKEN` 个在途请求，超出的调用排队等待并计入总超时。鉴权信息只发送到任务端点，不附带到结果下载请求。
- 识别区域由 `crop_box` 比例参数指定，默认右上角；取整规则和放大上传格式不随参数变化。
