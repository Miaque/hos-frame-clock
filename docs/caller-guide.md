...
- 支持的画面时间格式为 `YYYY-MM-DD HH:MM:SS`。OCR 将日期与时间之间的空格识别为 `-`、`.`、`:` 或省略空格时仍可解析；时分之间的 `.` 也可解析。除日期与时间紧邻时补一个空格外，`raw_text` 保留匹配到的 OCR 原文。不猜测修正多出的数字或 `O/0` 等字符，不补日期，不支持 ISO `T` 或毫秒格式。
