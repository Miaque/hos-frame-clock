...
- 支持的画面时间格式为 `YYYY-MM-DD HH:MM:SS`，日期与时间之间的空白可省略；缺少空白时（如 `2026-08-1505:20:17`），返回的 `raw_text` 会规范为 `2026-08-15 05:20:17`，已有空白保持不变。不猜测替换 `O/0` 等字符，不补日期，不支持 ISO `T` 或毫秒格式。
...
| `raw_text` | `str` | 匹配到的 OCR 文本；日期与时间紧邻时补一个空格，其他空白保留 |
